export  async function searchDocumentsRequest( context, keywords, nResults, nlp_server_address, timeout = 5000){
...
export async function get_papers_content(paper_list, nlp_server_address, timeout = 5000){
    const data = {
...
export async function extract_highlights_from_papers( paper_content_list, nlp_server_address, timeout = 5000){
...
export async function generate_citations_for_papers( paper_content_list, context, keywords, nlp_server_address, timeout = 5000 ){

    const context_list = paper_content_list.map( (item, idx)=>{ return context; } );
...
export async function callback_paginating_papers( paper_id_list, context, keywords, set_log_search_progress, nlp_server_address ){

    try{
        const paper_content_list = await get_papers_content( paper_id_list, nlp_server_address );
        set_log_search_progress("extracting highlights ...");
        const extracted_highlights_list = await extract_highlights_from_papers(paper_content_list, nlp_server_address);
        ...
        const gen_text_list = await generate_citations_for_papers( paper_content_list, context, keywords, nlp_server_address );
        ...
        set_log_search_progress("Error: please refresh the page and try again.");
        return null;
    }
...
export async function callback_search_pipeline(
    ...
    if ( context==="" && keywords ==="" ){
        ...
        try{
            ...
            const paper_id_list = await searchDocumentsRequest(context, keywords, nResults, nlp_server_address);
            
            const paper_info_list_slice = await callback_paginating_papers( 
                        paper_id_list.slice( new_paper_offset, new_paper_offset + num_papers_per_page ), context, keywords, set_log_search_progress, nlp_server_address );  
...
export async function title_generic_search( titles, nlp_server_address, timeout = 5000 ){
    ...
    const response = await fetch( nlp_server_address + "/ml-api/title-generic-search/v1.0", requestOptions);
    const response_data = await response.json();
    const response_data_response =  response_data["response"];  
    clearTimeout(timeout_id);
...
export async function text_processing(data, nlp_server_address, timeout = 5000){
...
export async function export_citation(paper_id_info, nlp_server_address, timeout = 5000){
    const data={
